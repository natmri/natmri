...
export class SQLiteStorageDatabase implements IStorageDatabase {
  ...
  constructor(private readonly path: string, private readonly options: ISQLiteStorageDatabaseOptions) {
  ...
  async close(recovery?: (() => Map<string, string>) | undefined): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      import('sqlite3').then((sqlite3) => {
        const connection: IDatabaseConnection = {
          db: new (this.logger.isTracing ? sqlite3.verbose().Database : sqlite3.default.Database)(path, (error: (Error & { code?: string }) | null) => {
            if (error)
              return (connection.db && error.code !== 'SQLITE_CANTOPEN')
  ...
  private execute(connection: IDatabaseConnection, sql: string): Promise<void> {
  ...
  private handleSQLiteError(connection: IDatabaseConnection, message: string) {
  ...
  private backup(): Promise<void> {
    ...
    return Promises.copyFile(this.path, backupPath)
  }
  ...
  private get(connection: IDatabaseConnection, sql: string): Promise<object> {
    return new Promise((resolve, reject) => {
      connection.db.get<object>(sql, (error, row) => {
        if (error) {
